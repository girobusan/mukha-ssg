...
function fixLink(to, from, lister, site_url) {
  ...
  if (to.startsWith("/+")) {
    ...
    // well formed?
    if (splitAt == -1) {
      log.warn("Malformed /+ link at", from, ":", linkstr);
      return to;
    }
    // to what file?
    let parts = [
    ...
    if (!parts[1]) {
      log.warn("Malformed /+ link, no value at", from);
      return to;
    }
    ...
    if (!f) {
      log.warn("Can not find target of /+ link from", from, ":", linkstr);
      return to;
    }
    // else {
    //   console.log("found...");
